...
namespace LibraryManageSystem
{
    class Borrower
    {
        ...
        public void Zhuce()
        {
            ...
            {
                PersonKu.borrowerlist.Add(new Person(strno, strname, strid));
            }
            else
            {
        ...
        public void BLogin()
        {
            ...
            var y = PersonKu.borrowerlist.Where(s => s.PersonName == name).FirstOrDefault();
            if (y != null)
            {
                Console.WriteLine("请输入您的学号/工号：");
                string number = Console.ReadLine();
                var q = PersonKu.borrowerlist.Where(s => s.PersonNo == number).FirstOrDefault();
                if(q!=null)
                {
            ...
            Console.WriteLine("请输入要选择的操作：\t1.借阅历史查询\t2.借阅书籍\t3.返回");
                ...
                switch (e)
                {
                    ...
                    case "1":
                        ...
                        break;
                    case "2":
                        Console.WriteLine("您的身份是“学生”,目前剩余借阅书籍为{0} 本",  (8 - Record.booklist1.Count));
        ...
        public void BookReturn()
        {
            ...
            string str = Console.ReadLine();
            var p = Record.booklist1.Where(s => s.Strbookname == str).FirstOrDefault();
            if (p != null)
            {
                foreach(Record bok1 in Record.booklist1)
                {
                    if(bok1.Strbookname==p.Strbookname)
                    Record.booklist1.Remove(bok1);
                    Console.WriteLine("还书成功");
                    return;
                }
                 
                int d = BookKu.booklist.Count+1;
                int f = Record.booklist1.Count - 1;
